...
ARG BABELFISH_VERSION=BABEL_4_3_0__PG_16_4
...
RUN apt-get update && \
    apt-get install -y postgresql-server-dev-all build-essential wget libsybdb5 freetds-dev unzip
...
RUN PG_CPPFLAGS='-I/usr/include -DENABLE_TDS_LIB' \
    SHLIB_LINK='-lsybdb -L/usr/lib64' make && \
    PG_CPPFLAGS='-I/usr/include -DENABLE_TDS_LIB' \
    SHLIB_LINK='-lsybdb -L/usr/lib64' make PG_CONFIG=${PG_CONFIG} install
...
RUN wget --no-check-certificate https://github.com/tds-fdw/tds_fdw/archive/v${TDS_FDW_VERSION}.tar.gz && \
    tar -xvzf v${TDS_FDW_VERSION}.tar.gz && \
    cd tds_fdw-${TDS_FDW_VERSION}/ && \
    make USE_PGXS=1 PG_CONFIG=/opt/babelfish/bin/pg_config && \
    make USE_PGXS=1 PG_CONFIG=/opt/babelfish/bin/pg_config install
